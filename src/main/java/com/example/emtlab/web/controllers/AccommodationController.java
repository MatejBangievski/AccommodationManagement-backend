...
public class AccommodationController {
    ...
    @Operation(summary = "Get all free accommodations", description = "Retrieves a list of all available accommodations.")
    @GetMapping("/findAllNonReserved")
    public List<DisplayAccommodationDto> findAllNonReserved() {
        return accommodationApplicationService.findAllNonReserved();
    }
    ...
    public ResponseEntity<DisplayAccommodationDto> save(@RequestBody CreateAccommodationDto createAccommodationDto) {
        return accommodationApplicationService.save(createAccommodationDto).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<DisplayAccommodationDto> update(@PathVariable Long id, @RequestBody CreateAccommodationDto createAccommodationDto) {
        return accommodationApplicationService.update(id, createAccommodationDto).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<DisplayAccommodationDto> reserveAccommodation(@PathVariable Long id, Authentication authentication) {
        ...
        return accommodationApplicationService.reserve(id, user.getUsername()).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<DisplayAccommodationDto> removeReservation(@PathVariable Long id) {
        return accommodationApplicationService.removeReservation(id).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<DisplayAccommodationDto> bookAccommodation(@PathVariable Long id, Authentication authentication) {
        User user = (User) authentication.getPrincipal();
        return accommodationApplicationService.book(id, user.getUsername()).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<DisplayAccommodationDto> completeStay(@PathVariable Long id) {
        return accommodationApplicationService.completeStay(id).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
